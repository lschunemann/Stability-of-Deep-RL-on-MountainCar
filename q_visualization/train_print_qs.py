...
from standard.helper import initialize_grids, initialize_q_table, initialize_state_dict, initialize_random_start, \
    epsilon_greedy_policy, get_closest_in_grid, evaluate
    ...
    for episode in range(n_training_episodes):
        ...
        for step in range(max_steps):
            ...
            if terminated:  # or truncated:
                if episode in print_at_epochs:
                    np.savetxt(f'data/q_{episode}.txt', q)
                    ...
                    plt.title(f"V scores for optimal action - episode - {episode}",
                              fontdict={'fontsize': 25})
                    plt.savefig(f'plots/v_values_{episode}.png')
                    plt.close()
                    ...
                    plt.xlabel("Position", fontsize=20)
                    plt.ylabel("Velocity", fontsize=20)
                    plt.title(f"Optimal action at state - episode: {episode}",
                              fontdict={'fontsize': 25})
                    ...
                    plt.savefig(f'plots/best_actions_{episode}.png')
                    plt.close()
...
print_at_epochs = [1, 4, 9, 25, 50, 100, 200, 500, 1000, 1999]
