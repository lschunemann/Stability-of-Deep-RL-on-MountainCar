...
def train(q, init_lr, name, rand_init=True):
    ...
    for episode in range(n_training_episodes):
        ...
        for step in range(max_steps):
            ...
            if name == 'fixed' or episode < 100:
                lr = init_lr
            elif name == 'step_100eps':
                lr = min_lr
            elif name == 'steps_100_500eps':
                if episode < 500:
                    lr = init_lr / 5
                else:
                    lr = min_lr
            elif name == 'steps_100_500_750eps':
                if episode < 500:
                    lr = init_lr / 5
                elif episode < 750:
                    lr = init_lr / 10
...
lrs = [(1, 'step_100eps'), (1, 'steps_100_500eps'), (1, 'steps_100_500_750eps'), (0.05, 'fixed')]

grid_x, grid_v = initialize_grids()
state_to_qtable = initialize_state_dict()

for init_lr, name in lrs:

    q_car = initialize_q_table(state_space, action_space)
    q_car, avg_rewards, total_steps = train(q_car, init_lr, name)
    ...
    plt.title(f"V scores for optimal action - rand_decay {name}", fontdict={'fontsize': 25})
    plt.savefig(f'plots/v_values_rand_decay_{name}.png')
    plt.close()
    ...
    plt.xlabel("Position", fontsize=20)
    plt.ylabel("Velocity", fontsize=20)
    plt.title(f"Optimal action at state - rand_decay {name}", fontdict={'fontsize': 25})
    ...
    plt.savefig(f'plots/best_actions_rand_decay_{name}.png')
    plt.close()

    # Plot total steps and average rewards for random init
    plot_steps(total_steps, f'rand_decay_{name}')
    plot_rewards(avg_rewards, f'rand_decay_{name}')

    # Save Q table
    np.savetxt(f'data/q_rand_decay_{name}.txt', q_car)
    np.array(avg_rewards)
    np.savetxt(f'data/avg_rewards_rand_decay_{name}.txt', avg_rewards)
    np.array(total_steps)
    np.savetxt(f'data/total_steps_rand_decay_{name}.txt', total_steps)
