...
import numpy as np
from helper import initialize_grids, initialize_q_table, initialize_state_dict, train
...
q_car, avg_rewards, total_steps = train(n_training_episodes, min_epsilon, max_epsilon, decay_rate, env, max_steps,
                                        q_car, grid_x, grid_v, learning_rate, state_to_qtable, gamma, True)

# Save Q table
np.savetxt('data/q_random.txt', q_car)
np.array(avg_rewards)
np.savetxt('data/avg_rewards_rand.txt', avg_rewards)
np.array(total_steps)
np.savetxt('data/total_steps_rand.txt', total_steps)
