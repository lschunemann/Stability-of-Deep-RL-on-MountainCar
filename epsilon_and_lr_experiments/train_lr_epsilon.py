...
def train(q, init_lr, k_lr, eps, decay, rand_init=True):
...
for init_lr, k_lr in lrs:
    for eps, decay in decays:
        q_car = initialize_q_table(state_space, action_space)
        q_car, avg_rewards, total_steps = train(q_car, init_lr, k_lr, eps, decay)
        ...
        plt.xlabel("Position", fontsize=20)
        plt.ylabel("Velocity", fontsize=20)
        plt.title(f"Optimal action at state - lr: ({init_lr}, {k_lr})  epsilon: ({eps}, {decay})", fontdict={'fontsize': 25})
