...
def train(q, eps, decay, rand_init=True):
    ...
    for episode in range(n_training_episodes):
        epsilon = min_epsilon + (eps - min_epsilon) * np.exp(-decay * episode)
        state = env.reset()
        if rand_init:
        ...
        if (episode + 1) % 10 == 0:
            ...
            reward_list = []
            print(f"decay: ({eps},{decay})\t episode: {episode}\t average reward: {avg_reward}\t "
...
decays = [(1, 0.005), (1, 0.01), (0.5, 0.005), (0.5, 0.01), (0.25, 0.005), (0.25, 0.01)]

for eps, decay in decays:
    q_car = initialize_q_table(state_space, action_space)
    q_car, avg_rewards, total_steps = train(q_car, eps, decay)
    ...
    plt.title(f"V scores for optimal action - rand_decay ({eps}, {decay})", fontdict={'fontsize': 25})
    plt.savefig(f'plots/v_values_rand_decay_{eps}_{decay}.png')
    plt.close()
    ...
    plt.xlabel("Position", fontsize=20)
    plt.ylabel("Velocity", fontsize=20)
    plt.title(f"Optimal action at state - rand_decay ({eps}, {decay})", fontdict={'fontsize': 25})
    ...
    plt.savefig(f'plots/best_actions_rand_decay_{eps}_{decay}.png')
    plt.close()

    # Plot total steps and average rewards for random init
    plot_steps(total_steps, f'rand_decay_{eps}_{decay}')
    plot_rewards(avg_rewards, f'rand_decay_{eps}_{decay}')

    # Save Q table
    np.savetxt(f'data/q_rand_decay_{eps}_{decay}.txt', q_car)
    np.array(avg_rewards)
    np.savetxt(f'data/avg_rewards_rand_decay_{eps}_{decay}.txt', avg_rewards)
    np.array(total_steps)
    np.savetxt(f'data/total_steps_rand_decay_{eps}_{decay}.txt', total_steps)
