...
eps = [(1, 'step_100eps'), (1, 'steps_100_500eps'), (1, 'steps_100_500_750eps'), (0.05, 'fixed')]

grid_x, grid_v = initialize_grids()
state_to_qtable = initialize_state_dict()

for init_eps, name in eps:

    q_car = initialize_q_table(state_space, action_space)
    q_car, avg_rewards, total_steps = train(q_car, init_eps, name)
    ...
    plt.title(f"V scores for optimal action - epsilon {name}", fontdict={'fontsize': 25})
    plt.savefig(f'plots/v_values_epsilon_{name}.png')
    plt.close()
    ...
    plt.xlabel("Position", fontsize=20)
    plt.ylabel("Velocity", fontsize=20)
    plt.title(f"Optimal action at state - epsilon {name}", fontdict={'fontsize': 25})
    ...
    plt.savefig(f'plots/best_actions_epsilon_{name}.png')
    plt.close()

    # Plot total steps and average rewards for random init
    plot_steps(total_steps, f'epsilon_{name}')
    plot_rewards(avg_rewards, f'epsilon_{name}')

    # Save Q table
    np.savetxt(f'data/q_epsilon_{name}.txt', q_car)
    np.array(avg_rewards)
    np.savetxt(f'data/avg_rewards_epsilon_{name}.txt', avg_rewards)
    np.array(total_steps)
    np.savetxt(f'data/total_steps_epsilon_{name}.txt', total_steps)
