...
car = TrainMountainCar(n_training_episodes=n_training_episodes, gamma=gamma, learning_rate=learning_rate,
                       ...
                       rainbow=rainbow, copy_target=copy_target, replay_size=replay_size,
...
torch.save(best_policy, 'data/Rainbow_DDQN_best.pth')
torch.save(final_policy, 'data/Rainbow_DDQN_final.pth')
np.savetxt(f'data/steps_Rainbow_DDQN.txt', total_steps_list)
np.savetxt(f'data/q_values_Rainbow_DDQN.txt', q_measures)
np.savetxt(f'data/eval_Rainbow_DDQN.txt', evaluations)
np.savetxt(f'data/td_error_Rainbow_DDQN.txt', td_error)
...
plt.xlabel('Episode')
plt.ylabel('Steps')
plt.title('Steps per Episode - Rainbow_DDQN')
plt.savefig('plots/steps_Rainbow_DDQN.png')
plt.close()
...
plt.savefig('plots/q_measures_Rainbow_DDQN.png')
plt.close()
